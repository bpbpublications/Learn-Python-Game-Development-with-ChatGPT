...
class Fighter:
    ...
    def move(self, screen_width, screen_height, target, round_over, state_change):
        SPEED = 10
        GRAVITY = 2
        dx = 0
        dy = 0
        ...
        #add a minor amount of healing
        self.health = min(self.health*1.001, 100)
        
        # Can only perform other actions if not currently attacking
        if not self.attacking and self.alive and not round_over:         
            action = state_change.get("action", "none")
            if action == "movement":
                dx = SPEED * state_change["dx"]
                self.running = True            
            elif action == "attacking":
                self.attack_type = state_change["attack_type"]
                self.attack(target)  # Assuming attack is a method in Fighter class
            elif action == "jumping" and not self.jump:
                self.vel_y = -30
                self.jump = True
