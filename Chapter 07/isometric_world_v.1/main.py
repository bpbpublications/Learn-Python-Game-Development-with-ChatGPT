...
def draw_tile_sprite(x, y, sprite, zoom):
    """Draw a tile sprite at (x, y) with given zoom."""
    width = sprite.get_width() * zoom
    h_off = (sprite.get_height() - 68) // 2  * zoom
...
def main():
    ...
    while True:
        ...
        pygame.draw.circle(screen, TOKEN_COLOR, (int(token_x), int(token_y)), int(TOKEN_SIZE * zoom))
