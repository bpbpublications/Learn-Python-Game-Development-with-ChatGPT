...
for _ in range(100):
    ...
    cloud_body = pymunk.Body(body_type=pymunk.Body.STATIC)
    cloud_body.position = x, y
    cloud_shape = pymunk.Circle(cloud_body, 50)  # Approximate the cloud as a circle for simplicity
    cloud_shape.friction = 1.0
    cloud_shape.collision_type = 1  # Unique identifier for clouds
    space.add(cloud_body, cloud_shape)
    clouds.append(cloud_shape)
    
def cloud_collision(arbiter, space, _):   
    global score 
    cloud_shape, car_shape = arbiter.shapes
    particle_system.emit_many(cloud_shape.body.position.x,
                              cloud_shape.body.position.y,
                              50,
                              variance=(100, 100),
                              speed=200,
                              particle_lifetime=2.0)
    space.remove(cloud_shape, cloud_shape.body)
    if cloud_shape in clouds:
        clouds.remove(cloud_shape)
    score += 100
    return False

space.add_collision_handler(1, 0).begin = cloud_collision  # 0 is the default collision type which our car uses
...
while True:
    ...
    # Replace the previous loop that drew clouds based on fixed positions
    for cloud_shape in clouds:
        pygame.draw.ellipse(screen, (255, 255, 255), (cloud_shape.body.position.x + offset_x - 50, cloud_shape.body.position.y - 25, 100, 50))
