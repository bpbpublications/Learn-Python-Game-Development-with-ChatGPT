...
space = pymunk.Space()
space.gravity = (0, 900)
space.damping = 0.9  # Apply damping to reduce bouncing effect
...
car_poly.friction = 0.7  # Increase friction
...
for offset in [-30, 30]:
    ...
    wheel_shape.friction = 1.0  # High friction for wheels
...
ground.friction = 1.0  # High friction for ground
space.add(ground)
...
while True:
    ...
    if keys[pygame.K_a]:
        car_body.apply_force_at_local_point((20000, 0), (0, 0))
    if keys[pygame.K_d]:
        car_body.apply_force_at_local_point((-20000, 0), (0, 0))
